...
class SocialMediaComplianceMailService
{
    ...
    public function notifyRequestSubmitted(SocialMediaComplianceRequest $request, User $subscriber): void
    {
        ...
        $copy = $this->emailTemplates->resolve($hub, 'smc_request_submitted', EmailTemplateCatalog::AUDIENCE_ADMIN, [
            'site_name' => $hub->name,
    ...
    {
        $hub = $this->hubs->current();
        $copy = $this->emailTemplates->resolve($hub, 'smc_approver_assigned', EmailTemplateCatalog::AUDIENCE_USER, [
            'request_id' => (string) $request->id,
    ...
    public function notifyStatusUpdated(
        ...
        $copy = $this->emailTemplates->resolve($hub, 'smc_status_updated', EmailTemplateCatalog::AUDIENCE_USER, [
            'request_id' => (string) $request->id,
            'status' => $status,
    ...
    public function notifyResubmitted(SocialMediaComplianceRequest $request, User $approver, User $subscriber): void
    {
        $hub = $this->hubs->current();
        $preview = $this->preview($request->currentVersionRow?->description);
        $copy = $this->emailTemplates->resolve($hub, 'smc_request_resubmitted', EmailTemplateCatalog::AUDIENCE_USER, [
            'request_id' => (string) $request->id,
            'user_name' => $subscriber->name,
            'user_email' => $subscriber->email,
            'site_name' => $hub->name,
