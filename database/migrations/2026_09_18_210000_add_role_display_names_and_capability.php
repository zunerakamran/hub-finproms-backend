...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('hubs', function (Blueprint $table) {
            $table->json('role_display_names')->nullable()->after('role_capabilities');
        });

        $matrix = app(CapabilitiesMatrixService::class);
        $key = 'dashboard_manage_role_display_names';
    ...
    public function down(): void
    {
        Hub::query()->orderBy('id')->each(function (Hub $hub) {
            ...
            foreach ($roleCaps as $role => $caps) {
                if (is_array($caps)) {
                    unset($roleCaps[$role]['dashboard_manage_role_display_names']);
                }
            }
            $checklist = is_array($hub->checklist) ? $hub->checklist : [];
            unset($checklist['dashboard_manage_role_display_names']);
            $hub->role_capabilities = $roleCaps;
            $hub->checklist = $checklist;
            $hub->role_display_names = null;
            $hub->save();
        });

        Schema::table('hubs', function (Blueprint $table) {
            $table->dropColumn('role_display_names');
        });
    }
};
